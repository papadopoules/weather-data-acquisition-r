...
ggplot(plot_df, aes(x = Month)) +
  geom_area(
    aes(y = Precipitation / scale_factor, fill = "Precipitation (mm)", group = 1),
    alpha = 0.3
  ) +

  geom_errorbar(
    aes(ymin = Low, ymax = High, color = "Temperature range (min/max)"),
    width = 0.3,
    linewidth = 0.8
  ) +
  ...
  scale_fill_manual(
    name = "",
    values = c("Precipitation (mm)" = "steelblue")
  ) +
  ...
  labs(
    title = "Annual Weather Averages in Guatemala City",
    subtitle = "Monthly precipitation with temperature range and mean temperature",
    caption = "Right axis: precipitation | Left axis: temperature",
    x = ""
  ) +
